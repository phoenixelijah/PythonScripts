...
ipAddress = input("Please enter your number:")
segments = 1
segLength = 0

if ipAddress == '':
    print('please to enter a value next time')
    exit()

for character in ipAddress:
    if character == '.':
        print("segment {} contains {} characters".format(segments, segLength))
        segments += 1
        segLength = 0
    else:
        segLength += 1

# unlesss the last character is a dot then we are missing final statement
if character != '.':
    print("segment {} contains {} characters".format(segments, segLength))
